//69_Print N-bit binary numbers having more 1s than 0s
...
class Solution {
  public:
    ...
    void prefixes(int n,string &s, char x, char y, int xcount,int ycount,vector<string> &ans)
    {
        if(s.length() == n)
        {
            ans.push_back(s);
        ...
        s.push_back(x);
        prefixes(n,s,x,y,xcount+1,ycount,ans);
        s.pop_back();
        
        if(xcount>ycount)
        {
            s.push_back(y);
            prefixes(n,s,x,y,xcount,ycount+1,ans);
            s.pop_back();
        }
    }
    ...
    vector<string> NBitBinary(int n) {
        ...
        int ycount = 0;
        string s = "";
        prefixes(n,s,x,y,xcount,ycount,ans);
        return ans;
    }
};